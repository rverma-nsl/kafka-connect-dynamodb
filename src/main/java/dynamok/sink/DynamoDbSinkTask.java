...
public class DynamoDbSinkTask extends SinkTask {
  ...
  private final Counter conditionalCheckFailed =
      DynamoDbSinkConnector.METRIC_REGISTRY.counter("ConditionalCheckFailed");
  private final Timer requestProcessingTimer =
      DynamoDbSinkConnector.METRIC_REGISTRY.timer("RequestProcessingTime");
  ...
  public void start(Map<String, String> props) {
    ...
    DynamoDbClientBuilder ddb = DynamoDbClient.builder().region(config.region);
    ...
    if (config.accessKeyId.value().isEmpty() || config.secretKey.value().isEmpty()) {
      ...
      log.debug(
          "AmazonDynamoDBClient created with AWS credentials from connector configuration");
    }
    client = ddb.build();
  ...
  public void put(Collection<SinkRecord> records) {
    ...
    for (final SinkRecord record : records) {
      ...
      try {
        ...
        if ("ValidationException".equalsIgnoreCase(e.awsErrorDetails().errorCode())) {
          producer.send(
              new ProducerRecord<>(
                  config.errorKafkaTopic,
                  "DynamoDbException" + record.key(),
  ...
  private PutItemRequest toPutRequest(
      SinkRecord record, DynamoConnectMetaData dynamoConnectMetaData) {
    ...
    if (config.kafkaCoordinateNames != null) {
      ...
      item.put(
          config.kafkaCoordinateNames.partition,
          AttributeValue.fromN(String.valueOf(record.kafkaPartition())));
      item.put(
          config.kafkaCoordinateNames.offset,
          AttributeValue.fromN(String.valueOf(record.kafkaOffset())));
    }
      ...
      pir.conditionExpression(dynamoConnectMetaData.getConditionalExpression())
          .expressionAttributeValues(
              AttributeValueConverter.toAttributeValueSchemaless(
                      dynamoConnectMetaData.getConditionalValueMap())
                  .m());
    }
  ...
  private void insert(
    ...
    if (!topAttributeName.isEmpty()) {
      ...
      throw new ConnectException(
          "No top attribute name configured for "
              + valueSource
              + ", and it could not be converted to Map: "
              + attributeValue);
    }
  }
  ...
  private String tableName(SinkRecord record) {
    ...
    //        return config.tableFormat.equalsIgnoreCase("${topic}") ?
    // config.tableFormat.replace("${topic}", record.topic()) : config.tableFormat;
  }
