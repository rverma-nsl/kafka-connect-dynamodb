...
public final class AttributeValueConverter {
      ...
      case MAP: {
        ...
        for (Map.Entry<?, ?> e : sourceMap.entrySet()) {
          attributesMap.put(
              primitiveAsString(
                  nullFallback(
                      e.getKey(), schema.keySchema().defaultValue())),
  ...
  public static AttributeValue toAttributeValueSchemaless(Object value) {
    ...
    if (value instanceof List) {
      // We could have treated it as NS/BS/SS if the list is homogeneous and a compatible
      // type, but
      // can't know for ane empty list
      return AttributeValue.fromL(
          ((List<?>) value)
              .stream()
    ...
    if (value instanceof Set) {
      ...
      if (firstItem instanceof byte[] || firstItem instanceof ByteBuffer) {
        return AttributeValue.fromBs(
            set.stream()
                .map(AttributeValueConverter::toByteBuffer)
                .collect(Collectors.toList()));
      }
